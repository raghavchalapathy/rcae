...
image_and_anamolies = {'image': 5,'anomalies1':3,'anomalies2':3,'imagecount': 500,'anomaliesCount': 150}
...
def visualise_anamolies_detected(testX,noisytestX,decoded,N,best_top10_keys,worst_top10_keys,lamda):
    ...
    print(basepath+'/best/')
    io.imsave(basepath+'/best/'+str(lamda)+'salt_p_denoising_ae_decode.png', img)
    ...
    io.imsave(basepath+'/worst/'+str(lamda)+'salt_p_denoising_ae_decode.png', img)
...
print('P@10  = %1.4f +- %1.4f' % (1-np.mean(prec), np.std(prec)/np.sqrt(TRIALS)))
